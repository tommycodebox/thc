...
pub fn create_extrinsic(
	...
	function: impl Into<thc_runtime::Call>,
	nonce: Option<u32>,
) -> thc_runtime::UncheckedExtrinsic {
	...
	let period = thc_runtime::BlockHashCount::get()
		.checked_next_power_of_two()
		.map(|c| c / 2)
		.unwrap_or(2) as u64;
	let tip = 0;
	let extra: thc_runtime::SignedExtra = (
		frame_system::CheckSpecVersion::<thc_runtime::Runtime>::new(),
		frame_system::CheckTxVersion::<thc_runtime::Runtime>::new(),
		frame_system::CheckGenesis::<thc_runtime::Runtime>::new(),
		frame_system::CheckEra::<thc_runtime::Runtime>::from(generic::Era::mortal(
			period,
			best_block.saturated_into(),
		)),
		frame_system::CheckNonce::<thc_runtime::Runtime>::from(nonce),
		frame_system::CheckWeight::<thc_runtime::Runtime>::new(),
		pallet_transaction_payment::ChargeTransactionPayment::<thc_runtime::Runtime>::from(tip),
	);

	let raw_payload = thc_runtime::SignedPayload::from_raw(
		function.clone(),
		extra.clone(),
		(
			thc_runtime::VERSION.spec_version,
			thc_runtime::VERSION.transaction_version,
			genesis_hash,
			best_hash,
			(),
			(),
	...
	thc_runtime::UncheckedExtrinsic::new_signed(
		function.clone(),
		sp_runtime::AccountId32::from(sender.public()).into(),
		thc_runtime::Signature::Sr25519(signature.clone()),
		extra.clone(),
	)
}
